...
namespace Polycode.NostalgicPlayer.Ports.LibFlac.Private
{
	...
	internal interface ILpc
	{
		...
		/// Asserts that lag > 0.
		/// </summary>
		void Compute_Autocorrelation(Flac__real[] data, uint32_t data_Len, uint32_t lag, double[] autoc);
		...
		/// </summary>
		void Compute_Residual_From_Qlp_Coefficients_Wide(Flac__int32[] data, uint32_t data_Offset, uint32_t data_Len, Flac__int32[] qlp_Coeff, uint32_t order, int lp_Quantization, Flac__int32[] residual);
