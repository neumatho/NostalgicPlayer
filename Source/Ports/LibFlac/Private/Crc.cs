...
namespace Polycode.NostalgicPlayer.Ports.LibFlac.Private
{
	...
	internal static class Crc
	{
		...
		/// </summary>
		private static readonly Flac__uint8[] crc8_Table =
		{
		...
		/// </summary>
		private static readonly Flac__uint16[,] crc16_Table =
		{
			{
		...
		public static Flac__uint16 Flac__Crc16(Span<Flac__byte> data, uint32_t len)
		{
			...
			while (len >= 8)
			{
				...
				crc = (Flac__uint16)(crc16_Table[7, crc >> 8] ^ crc16_Table[6, crc & 0xff] ^
				      crc16_Table[5, data[offset + 2]] ^ crc16_Table[4, data[offset + 3]] ^
				      crc16_Table[3, data[offset + 4]] ^ crc16_Table[2, data[offset + 5]] ^
				      crc16_Table[1, data[offset + 6]] ^ crc16_Table[0, data[offset + 7]]);
			...
			while (len-- != 0)
				crc = (Flac__uint16)((crc << 8) ^ crc16_Table[0, (crc >> 8) ^ data[offset++]]);
		...
		public static Flac__uint16 Flac__Crc16_Update(Flac__byte data, Flac__uint16 crc)
		{
			return (Flac__uint16)(((crc << 8) & 0xffff) ^ crc16_Table[0, (crc >> 8) ^ data]);
		}
		...
		/// </summary>
		/********************************************************************/
		public static Flac__uint16 Flac__Crc16_Update_Words32(Flac__uint32[] words, uint32_t offset, uint32_t len, Flac__uint16 crc)
		{
			while (len >= 2)
			{
				crc ^= (Flac__uint16)(words[offset] >> 16);

				crc = (Flac__uint16)(crc16_Table[7, crc >> 8] ^ crc16_Table[6, crc & 0xff] ^
				                     crc16_Table[5, (words[offset] >> 8) & 0xff] ^ crc16_Table[4, words[offset] & 0xff] ^
				                     crc16_Table[3, words[offset + 1] >> 24] ^ crc16_Table[2, (words[offset + 1] >> 16) & 0xff] ^
				                     crc16_Table[1, (words[offset + 1] >> 8) & 0xff] ^ crc16_Table[0, words[offset + 1] & 0xff]);
			...
			if (len != 0)
			{
				...
				crc = (Flac__uint16)(crc16_Table[3, crc >> 8] ^ crc16_Table[2, crc & 0xff] ^
				                     crc16_Table[1, (words[offset] >> 8) & 0xff] ^ crc16_Table[0, words[offset] & 0xff]);
			}
		...
		/// </summary>
		/********************************************************************/
		public static Flac__uint16 Flac__Crc16_Update_Words64(Flac__uint64[] words, uint32_t offset, uint32_t len, Flac__uint16 crc)
		{
			while (len-- != 0)
			{
				crc ^= (Flac__uint16)(words[offset] >> 48);

				crc = (Flac__uint16)(crc16_Table[7, crc >> 8] ^ crc16_Table[6, crc & 0xff] ^
				                     crc16_Table[5, (words[offset] >> 40) & 0xff] ^ crc16_Table[4, (words[offset] >> 32) & 0xff] ^
				                     crc16_Table[3, (words[offset] >> 24) & 0xff] ^ crc16_Table[2, (words[offset] >> 16) & 0xff] ^
				                     crc16_Table[1, (words[offset] >> 8) & 0xff] ^ crc16_Table[0, words[offset] & 0xff]);
