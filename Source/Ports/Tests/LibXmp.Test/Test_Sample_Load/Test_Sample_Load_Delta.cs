...
namespace Polycode.NostalgicPlayer.Ports.Tests.LibXmp.Test.Test_Sample_Load
{
	...
	public partial class Test_Sample_Load
	{
		...
		public void Test_Sample_Load_Delta()
		{
			...
			int8[] conv_R0 = new int8[10] { 0, 1, 3, 6, 10, 15, 21, 14, 22, -7 };
			...
			uint16[] conv_R1 = new uint16[10] { 0, 1, 3, 6, 10, 15, 21, 14, 22, 65529 };

			Module_Data m = new Module_Data();

			xxs.Len = 10;
			Sample.LibXmp_Load_Sample(m, null, Sample_Flag.NoLoad | Sample_Flag.Diff, xxs, MemoryMarshal.Cast<int8, uint8>(buffer0));
			Assert.IsTrue(ArrayHelper.ArrayCompare(xxs.Data, xxs.DataOffset, MemoryMarshal.Cast<int8, uint8>(conv_R0).ToArray(), 0, 10), "Invalid 8-bit conversion");
