...
namespace Polycode.NostalgicPlayer.Ports.Tests.LibXmp.Test.Test_No_Note
{
	...
	public partial class Test_No_Note
	{
		...
		/********************************************************************/
		[TestMethod]
		public void Test_No_Note_Valid_Ins_Ft2()
		{
			...
			New_Event(opaque, 0, 3, 0, 0, 0, 0, Effects.Fx_VolSet, Set_Vol, Effects.Fx_SetPan, Set_Pan);
			New_Event(opaque, 0, 4, 0, Constants.Xmp_Key_Off, Ins_1, 0, 0x00, 0, 0, 0);
			...
			// FIXME: move to test_keyoff_valid_ins_ft2.c
			opaque.Xmp_Play_Frame();

			Check_On(xc, vi, Key_C5, Ins_0, Set_Vol, Set_Pan, Ins_0_Fade, "row 3");

			opaque.Xmp_Play_Frame();

			// Row 4: Valid instrument with key off (FT2)
			//
			// This behaves the same as valid instrument with no note, except
			// it also acts like key off
			opaque.Xmp_Play_Frame();

			Check_On(xc, vi, Key_C5, Ins_0, -1, Ins_0_Sub_0_Pan, Ins_0_Fade, "row 4");
