...
namespace Polycode.NostalgicPlayer.Ports.Tests.LibXmp.Test.Test_Effect
{
	...
	public partial class Test_Effect
	{
		...
		/********************************************************************/
		[TestMethod]
		public void Test_Effect_1_Slide_Up()
		{
			...
			// Standard pitch bend
			New_Event(opaque, 0, 0, 0, 49, 1, 0, 1, 2, 0, 0);
			for (c_int i = 1; i < 60; i++)
				New_Event(opaque, 0, i, 0, 0, 0, 0, 1, 0, 0, 0);
			...
			New_Event(opaque, 0, 0, 0, 49, 1, 0, 1, 0xf2, 0, 0);
			for (c_int i = 1; i < 60; i++)
				New_Event(opaque, 0, i, 0, 0, 0, 0, 1, 0, 0, 0);
			...
			opaque.Xmp_Play_Frame();
			opaque.Xmp_Get_Frame_Info(out info);
			Assert.AreEqual(856, Period(info), "Slide error (no fine slide)");

			opaque.Xmp_Play_Frame();
			opaque.Xmp_Get_Frame_Info(out info);
			Assert.AreEqual(614, Period(info), "Slide error (no fine slide)");

			opaque.Xmp_Play_Frame();
			opaque.Xmp_Get_Frame_Info(out info);
			Assert.AreEqual(372, Period(info), "Slide error (no fine slide)");
			...
			New_Event(opaque, 0, 0, 0, 49, 1, 0, 1, 0xe4, 0, 0);
			for (c_int i = 1; i < 60; i++)
				New_Event(opaque, 0, i, 0, 0, 0, 0, 1, 0, 0, 0);
