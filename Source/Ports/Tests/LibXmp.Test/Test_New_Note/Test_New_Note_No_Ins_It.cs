...
namespace Polycode.NostalgicPlayer.Ports.Tests.LibXmp.Test.Test_New_Note
{
	...
	public partial class Test_New_Note
	{
		...
		/********************************************************************/
		[TestMethod]
		public void Test_New_Note_No_Ins_It()
		{
			...
			New_Event(opaque, 0, 2, 0, Key_D4, 0, 0, 0x00, 0, 0, 0);
			...
			// Row 2: No instrument with new note (IT)
			//
			// When no instrument and a new note is set, IT plays
			// the new note with old sample and current volume
			opaque.Xmp_Play_Frame();

			Check_New(xc, vi, Key_D4, Ins_0, Set_Vol, -1/*FIXME: Set_Pan*/, Ins_0_Fade, "row 2");
