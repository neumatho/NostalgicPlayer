...
namespace Polycode.NostalgicPlayer.Ports.Tests.LibXmp.Test.Test_Porta
{
	...
	public partial class Test_Porta
	{
		...
		/********************************************************************/
		[TestMethod]
		public void Test_Porta_No_Ins_Ft2()
		{
			...
			New_Event(opaque, 0, 2, 0, Key_D4, 0, 0, Effects.Fx_TonePorta, 4, 0, 0);
			New_Event(opaque, 0, 3, 0, 0, Ins_0, 0, 0x00, 0, 0, 0);	// FT2 only
			...
			// Row 2: No instrument with tone portamento (FT2)
			//
			// When no instrument is played with tone portamento,
			// FT2 keeps playing the current sample
			opaque.Xmp_Play_Frame();

			Check_On(xc, vi, Key_C5, Ins_0, Set_Vol, Set_Pan, Ins_0_Fade, "row 2");

			opaque.Xmp_Play_Frame();

			// Row 3: Instrument without note still resets vol/pan (FT2)
			//
			// This should still be using instrument 0 for volume/pan
			opaque.Xmp_Play_Frame();

			Check_On(xc, vi, Key_C5, Ins_0, Ins_0_Sub_0_Vol, Ins_0_Sub_0_Pan, Ins_0_Fade, "row 3");
