...
namespace Polycode.NostalgicPlayer.Ports.LibXmp.Loaders
{
	...
	internal class Dsm_Load : IFormatLoader
	{
		...
		private Dsm_Song mh;
		private Dsm_Note[] dsmBuf;

		private uint32 blockId;
		private uint32 blockLn;
		private uint32 blockLp;
		...
		public c_int Test(Hio f, out string t, c_int start)
		{
			...
			uint8[] buf = new uint8[12];

			if (f.Hio_Read(buf, 1, 12) < 12)
				return -1;

			if ((Encoding.ASCII.GetString(buf, 0, 4) != "RIFF") || (Encoding.ASCII.GetString(buf, 8, 4) != "DSMF"))
				return -1;
		...
		public c_int Loader(Module_Data m, Hio f, c_int start)
		{
			...
			mh = new Dsm_Song();
			dsmBuf = ArrayHelper.InitializeArray<Dsm_Note>(MaxChan * 64);
			...
			try
			{
				...
				Array.Copy(mh.Orders, mod.Xxo, mod.Len);
		...
		/// </summary>
		/********************************************************************/
		private bool GetBlockHeader(Hio f)
		{
		...
		/// </summary>
		/********************************************************************/
		private bool ReadPattern(Hio f)
		{
			int row = 0;
		...
		/// </summary>
		/********************************************************************/
		private void ConvertTrack(Dsm_Note[] tr, int offset, Xmp_Event[] events)
		{
			for (int t = 0; t < 64; t++)
			{
