...
namespace Polycode.NostalgicPlayer.Agent.ModuleConverter.ProWizardConverter.Formats
{
	...
	/// HCD Protector
	/// </summary>
	internal class HcdProtectorFormat : ProWizardForPcBase
	{
		...
		/// </summary>
		/********************************************************************/
		public override string OriginalFormat => Resources.IDS_PROWIZ_NAME_AGENT56;
		#endregion
		...
		protected override bool CheckModule(ModuleStream moduleStream)
		{
			...
			for (int i = 0; i < 31; i++)
			{
				...
				ushort loopStart = (ushort)((moduleStream.Read_B_UINT16() * 2) - 0x68a);
		...
		protected override IEnumerable<SampleInfo> GetSamples(ModuleStream moduleStream)
		{
			for (int i = 0; i < 31; i++)
			{
				...
				byte fineTune = moduleStream.Read_UINT8();
				byte volume = moduleStream.Read_UINT8();
				ushort loopStart = (ushort)(moduleStream.Read_B_UINT16() - 0x345);
		...
		protected override IEnumerable<byte[]> GetPatterns(ModuleStream moduleStream)
		{
			...
			for (int i = 0; i < numberOfPatterns; i++)
			{
				for (int j = 0; j < 64 * 4; j++)
				{
					...
					pattern[j * 4] = (byte)(byt1 >> 3);
					pattern[j * 4 + 1] = (byte)((byt2 >> 3) | (byt1 << 5));
					pattern[j * 4 + 2] = (byte)((byt3 >> 3) | (byt2 << 5));
					pattern[j * 4 + 3] = (byte)((byt4 >> 3) | (byt3 << 5));
				}
