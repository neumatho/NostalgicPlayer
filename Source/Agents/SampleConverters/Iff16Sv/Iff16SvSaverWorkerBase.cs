...
namespace Polycode.NostalgicPlayer.Agent.SampleConverter.Iff16Sv
{
	...
	/// </summary>
	internal abstract class Iff16SvSaverWorkerBase : Iff16SvLoaderWorkerBase, ISampleSaverAgent
	{
		...
		/// </summary>
		/********************************************************************/
		public string FileExtension => "16sv";
		...
		public void SaveHeader(Stream stream)
		{
			using (WriterStream writerStream = new WriterStream(stream))
			{
				...
				writerStream.Write_B_UINT32(0x31365356);						// 16SV
				...
				strBuf = encoder.GetBytes(Resources.IDS_IFF16SV_ANNO);
				strLen = strBuf.Length;
