...
namespace NostalgicPlayer.Kit.C.Test.String
{
	...
	/// </summary>
	[TestClass]
	public class Strtoll : TestStringBase
	{
		/********************************************************************/
		/// <summary>
		/// 
		...
		{
			Expect_Parse_Exact("9223372036854775808", 10, c_long_long.MaxValue, 19, true, "overflow just over LLONG_MAX");
			Expect_Parse_Exact("-9223372036854775809", 10, c_long_long.MinValue, 20, true, "underflow just below LLONG_MIN");

			Expect_Parse_Exact("9999999999999999999999999", 10, c_long_long.MaxValue, 25, true, "overflow many 9s");
			Expect_Parse_Exact("-9999999999999999999999999", 10, c_long_long.MinValue, 26, true, "underflow many 9s");
		}
		...
		/// </summary>
		/********************************************************************/
		private void Expect_Parse_Exact(string input, c_int @base, c_long_long expect_Val, c_long expect_End_Off, bool expect_Error, string label)
		{
			CPointer<char> inputPtr = input.ToCharPointer();

			c_long_long got = CString.strtoll(inputPtr, out CPointer<char> end, @base, out bool error);
		...
		private void Expect_No_Conv(string input, c_int @base, string label)
		{
			...
			c_long_long got = CString.strtoll(inputPtr, out CPointer<char> end, @base, out _);
