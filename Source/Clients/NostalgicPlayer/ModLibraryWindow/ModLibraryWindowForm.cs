...
namespace Polycode.NostalgicPlayer.Client.GuiPlayer.ModLibraryWindow
{
    ...
    public partial class ModLibraryWindowForm : WindowFormBase, IModLibraryWindowApi
    {
        ...
        public void ReloadChanges()
        {
            ...
            var currentPath = GetCurrentModLibraryPath();
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (service != null)
                    {
                        ...
                        var relativePath = data.GetRelativePathFromService(currentPath, service);
        ...
        private void ModuleListView_RetrieveVirtualItem(object sender, RetrieveVirtualItemEventArgs e)
        {
            ...
            var item = new ListViewItem($"{icon} {entry.Name}");
            ...
            if (flatViewCheckBox.Checked && !entry.IsDirectory)
            {
                ...
                var pathOnly = lastSlash >= 0 ? entry.FullPath.Substring(0, lastSlash) : string.Empty;
                ...
                if (!data.IsOfflineMode)
                {
                    ...
                    if (service != null)
                    {
                        var relativePath = data.GetRelativePathFromService(entry.FullPath, service);
                        // Remove filename from relative path
                        var relativeLastSlash = relativePath.LastIndexOf('/');
                        var relativePathOnly = relativeLastSlash >= 0 ? relativePath.Substring(0, relativeLastSlash) : string.Empty;
                        // Add service name before path
                        pathOnly = string.IsNullOrEmpty(relativePathOnly) ? service.DisplayName : $"{service.DisplayName}/{relativePathOnly}";
                    }
                }
            ...
            if (entry.IsDirectory)
            {
                ...
                var fileCount = CountFilesInTree(entry);
        ...
        private async void ModuleListView_DoubleClick(object sender, EventArgs e)
        {
            ...
            var index = moduleListView.SelectedIndices[0];
        ...
        private void ClearDatabaseItem_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                try
                {
                    ...
                    var allmodsTxtPath = Path.Combine(cacheDirectory, "ModLand", "allmods.txt");
                    ...
                    var allmodsZipPath = Path.Combine(cacheDirectory, "ModLand", "allmods.zip");
                ...
                catch (Exception ex)
                {
                    ...
                    MessageBox.Show(string.Format(Resources.IDS_MODLIBRARY_MSGBOX_FAILED_CLEAR_DB, ex.Message), Resources.IDS_MODLIBRARY_MSGBOX_TITLE_CLEAR_ERROR, MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        private void SearchDebounceTimer_Tick(object sender, EventArgs e)
        {
            ...
            var searchMode = (SearchMode)searchModeComboBox.SelectedIndex;
        ...
        private void FlatViewCheckBox_CheckedChanged(object sender, EventArgs e)
        {
            ...
            ShowListViewStatus(flatViewCheckBox.Checked ? Resources.IDS_MODLIBRARY_STATUS_BUILDING_FLAT_VIEW : Resources.IDS_MODLIBRARY_STATUS_BUILDING_TREE_VIEW);
            RebuildTree();
        }
        ...
        private void ModeTabControl_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            var isOffline = modeTabControl.SelectedIndex == 1;
            ...
            if (data.IsOfflineMode != isOffline)
            {
                ...
                ShowListViewStatus(isOffline ? Resources.IDS_MODLIBRARY_STATUS_SWITCHING_TO_LOCAL : Resources.IDS_MODLIBRARY_STATUS_SWITCHING_TO_ONLINE);
        ...
        private void OnDataLoaded(object sender, EventArgs e)
        {
            ...
            {
                var totalFiles = data.CountTotalFilesInFilteredCache();
                statusLabel.Text = string.Format(Resources.IDS_MODLIBRARY_STATUS_FOUND_FILES, totalFiles, data.SearchFilter);
            }
            else
            {
                // Full tree loaded
        ...
        private void DeleteItem_Click(object sender, EventArgs e)
        {
            ...
            var selectedEntries = new List<TreeNode>();
            ...
            {
                var fileCount = selectedEntries.Count(e => !e.IsDirectory);
                var folderCount = selectedEntries.Count(e => e.IsDirectory);
            ...
            if (result == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    var modulesBasePath = GetModLibraryModulesPath();
                    ...
                    foreach (var entry in selectedEntries)
                    {
                        try
                        {
                            ...
                            var localPath = Path.Combine(modulesBasePath, entry.FullPath.Replace('/', Path.DirectorySeparatorChar));
                    ...
                    else
                    {
                        ...
                        MessageBox.Show(string.Format(Resources.IDS_MODLIBRARY_MSGBOX_DELETE_COMPLETED_WITH_ERRORS, deletedCount, errorCount, lastError), Resources.IDS_MODLIBRARY_MSGBOX_TITLE_DELETE_ERRORS, MessageBoxButtons.OK,
                            MessageBoxIcon.Warning);
                    }
                }
                catch (Exception ex)
        ...
        private void JumpToFolderItem_Click(object sender, EventArgs e)
        {
            ...
            var folderPath = lastSlash >= 0 ? relativePath.Substring(0, lastSlash) : string.Empty;

            // Build full folder path
            var fullFolderPath = service.RootPath + folderPath;
        ...
        private string GetCurrentModLibraryPath()
        {
            ...
            var pathSettings = new PathSettings(userSettings);
        ...
        {
            var title = Resources.IDS_MODLIBRARY_TITLE;
        ...
        private bool ShouldIncludeFile(string nameWithPath)
        {
            ...
            var fileName = nameWithPath.Substring(nameWithPath.LastIndexOf('/') + 1);
        ...
        /// </summary>
        /********************************************************************/
        private string GetLocalPathForEntry(TreeNode entry) => data.IsOfflineMode ? GetLocalPathForLocalMode(entry) : GetLocalPathForOnlineMode(entry);
        ...
        private string GetLocalPathForOnlineMode(TreeNode entry)
        {
            ...
            var modulesBasePath = GetModLibraryModulesPath();
            return Path.Combine(modulesBasePath, service.FolderName, relativePath.Replace('/', Path.DirectorySeparatorChar));
        }
        ...
        {
            // ModLand service
            var modlandCacheDir = Path.Combine(cacheDirectory, "ModLand");
        ...
        private async void CheckExistingDatabase()
        {
            ...
            if (File.Exists(allmodsTxtPath))
            {
                var fileInfo = new FileInfo(allmodsTxtPath);
                ...
                {
                    var result = MessageBox.Show(
                        string.Format(Resources.IDS_MODLIBRARY_MSGBOX_DATABASE_OLD, (int)(DateTime.Now - modland.LastUpdate).TotalDays),
        ...
        private async Task DownloadModLandDatabase()
        {
            ...
            var modlandCacheDir = Path.Combine(cacheDirectory, "ModLand");
            var allmodsZipPath = Path.Combine(modlandCacheDir, "allmods.zip");
            var allmodsTxtPath = Path.Combine(modlandCacheDir, "allmods.txt");
            ...
            try
            {
                ...
                {
                    // Download the file
                    var fileBytes = await client.GetByteArrayAsync(AllmodsZipUrl);
        ...
        private void ParseModLandDatabase(ModuleService service, string allmodsTxtPath)
        {
            ...
            foreach (var line in File.ReadLines(allmodsTxtPath))
            {
                ...
                var nameWithPath = string.Join(" ", parts.Skip(1));
        ...
        private void ShowListViewStatus(string message)
        {
            ...
            {
                new() { Name = message, IsDirectory = false, FullPath = string.Empty, Size = 0, ServiceId = string.Empty }
            };
        ...
        private void LoadCurrentDirectory(string itemToSelect = null)
        {
            ...
            var sortOrder = (FlatViewSortOrder)settings.FlatViewSortOrder;
        ...
        private void UpdateBreadcrumbAndParentButton()
        {
            ...
            var folderCount = currentEntries.Count(e => e.IsDirectory);
            var fileCount = currentEntries.Count - folderCount;
            ...
            {
                parentButton.Enabled = false;
                var text = !string.IsNullOrEmpty(data.SearchFilter)
                    ? string.Format(Resources.IDS_MODLIBRARY_BREADCRUMB_FLAT_VIEW_SEARCH, data.SearchFilter, currentEntries.Count)
                    : string.Format(Resources.IDS_MODLIBRARY_BREADCRUMB_FLAT_VIEW_ALL, currentEntries.Count);
                AddBreadcrumbLabel(text);
            }
        ...
        private void BuildClickableBreadcrumb(int folderCount, int fileCount)
        {
            ...
            if (currentPath.EndsWith("://"))
            {
                ...
                var service = data.GetServiceFromPath(currentPath);
                var serviceName = service?.DisplayName ?? currentPath.Substring(0, currentPath.Length - 3);
            ...
            for (var i = 0; i < parts.Length; i++)
            {
                ...
                if (parts[i].EndsWith(":"))
                {
                    ...
                    var serviceName = service?.DisplayName ?? parts[i].TrimEnd(':');
        ...
        private void BuildClickablePath(string path)
        {
            ...
            if (path.EndsWith("://"))
            {
                ...
                var serviceName = service?.DisplayName ?? path.Substring(0, path.Length - 3);
        ...
        private async Task DownloadAndPlayModule(TreeNode entry)
        {
            try
            {
                ...
                var modulesBasePath = GetModLibraryModulesPath();
                var localPath = Path.Combine(modulesBasePath, service.FolderName, relativePath.Replace('/', Path.DirectorySeparatorChar));
                ...
                if (!File.Exists(localPath))
                {
                    ...
                    if (service.Id == "modland")
                    {
                        using (var client = new HttpClient())
                        {
                            ...
                            var downloadUrl = ModlandModulesUrl + encodedPath;
                            ...
                            {
                                var smplFileName = "smpl" + entry.Name.Substring(4); // Replace "mdat" with "smpl"
                                var smplRelativePath = relativePath.Substring(0, relativePath.LastIndexOf('/') + 1) + smplFileName;
                                var smplLocalPath = Path.Combine(localDirectory, smplFileName);
                                // URL-encode the smpl path too
                                var smplEncodedPath = string.Join("/", smplRelativePath.Split('/').Select(Uri.EscapeDataString));
                                var smplDownloadUrl = ModlandModulesUrl + smplEncodedPath;

                                try
                                {
                                    statusLabel.Text = string.Format(Resources.IDS_MODLIBRARY_STATUS_DOWNLOADING_SAMPLE, smplFileName);
                                    var smplBytes = await client.GetByteArrayAsync(smplDownloadUrl);
                    ...
                    statusLabel.Text = string.Format(Resources.IDS_MODLIBRARY_STATUS_DOWNLOADED_FILE, entry.Name, entry.Size);
            ...
            {
                statusLabel.Text = string.Format(Resources.IDS_MODLIBRARY_STATUS_ERROR_PREFIX, ex.Message);
                MessageBox.Show(string.Format(Resources.IDS_MODLIBRARY_MSGBOX_FAILED_DOWNLOAD_MODULE, ex.Message), Resources.IDS_MODLIBRARY_MSGBOX_TITLE_DOWNLOAD_ERROR, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private async Task PlayModuleIfExists(TreeNode entry)
        {
            try
            {
                var localPath = GetLocalPathForEntry(entry);
                if (localPath == null)
                {
                    return;
                }
                ...
                else
                {
                    ...
                    MessageBox.Show(string.Format(Resources.IDS_MODLIBRARY_MSGBOX_FILE_NOT_DOWNLOADED, entry.Name), Resources.IDS_MODLIBRARY_MSGBOX_TITLE_FILE_NOT_FOUND, MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
                statusLabel.Text = string.Format(Resources.IDS_MODLIBRARY_STATUS_ERROR_PREFIX, ex.Message);
                MessageBox.Show(string.Format(Resources.IDS_MODLIBRARY_MSGBOX_FAILED_PLAY_MODULE, ex.Message), Resources.IDS_MODLIBRARY_MSGBOX_TITLE_PLAY_ERROR, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void ScanLocalFiles()
        {
            ...
            var modulesDir = GetModLibraryModulesPath();
            ...
            foreach (var filePath in allFiles)
            {
                ...
                var fullRelativePath = filePath.Substring(modulesDir.Length + 1).Replace(Path.DirectorySeparatorChar, '/');
        ...
        private void AddFileToLocalCache(ModuleService service, string nameWithPath, long fileSize)
        {
            ...
            var fullPath = $"{service.FolderName}/{nameWithPath}";
